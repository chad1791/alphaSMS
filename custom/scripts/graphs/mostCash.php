<?php 
	...
	$restaurantIds = getAllRestaurantIdsByArea($location);
	...
	for($i=0;$i<count($restaurantIds);$i++){
		...
		if($start_date != 'All'){
			...
			$sql = "SELECT SUM(total) AS total FROM `remote_delivery_items` `rd` LEFT JOIN `remote_delivery_details` `rdd` ON `rd`.`invoice_id` = `rdd`.`invoice_id` WHERE `rdd`.`dateToday`>='".$start_date."' AND `rdd`.`dateToday`<='".$end_date."' AND restaurant_id='".$restaurantIds[$i]['id']."'";
		}
		else{
			$sql = "SELECT SUM(total) AS total FROM `remote_delivery_items` WHERE restaurant_id='".$restaurantIds[$i]['id']."'";
		}
		...
		$decimal = number_format((float)$rowsCount['total'], 1, '.', '');
		...
		if($rowsCount['total'] > 0){
			...
			$row = array('country'=>$shortName,'visits'=>$decimal,'color'=>$colors[$i],'completeName'=>$restName);
